...
//
// hist (complément)
// - 06/11/2018 (diam) : ajout représentation chaine du type LONG
// - 02/02/2018 (diam) : ajout du type LONG en plus de INTEGER
// - 30/11/2017 (diam) : ajout de add_desc() pour compléter une description
//   Le texte est précédé d'une saut de ligne et de l'indentation nécessaire.
// - 06/12/2016 (diam) : remplacement des NULL par des nullptr
...
class OptionParser {
  ...
  class Option {
   public:
    ...
    Option(string id, SpecType type = STRING) {
      ...
      this->ref_option = nullptr;

      this->bool_ref_variable = nullptr;
      this->int_ref_variable = nullptr;
      this->long_ref_variable = nullptr;
      this->double_ref_variable = nullptr;
      this->string_ref_variable = nullptr;
    }
    ~Option() {
      delete this->keys;
      delete this->abbrevs;
      delete this->ref_option;
      delete this->bool_ref_variable;
      delete this->int_ref_variable;
      delete this->long_ref_variable;
      delete this->double_ref_variable;
      delete this->string_ref_variable;
    }
    ...
    // SWITCH ; car se serait très lourd de récupérer la référence sur l'option
    // de type ABBREV crée par cette méthode add_abbrev de façon à en modifier
    ...
    Option *add_abbrev(string key, long value, bool internal = false) {
      stringstream buf;
      buf << value;
      return add_abbrev(key, buf.str(), internal);
    }
    ...
    // c-style-strings-to-stdstring-conversion-clarification
    Option *set_value(char const *chars) { return set_value(string(chars)); }
    Option *set_value(const string &value) {
      ...
      if (this->type == BOOL || this->type == SWITCH) {
        ...
        *this->string_ref_variable = value;
        ...
        cerr << "ERREUR : méthode set_value() non implantée pour "
             << "le type \"" << type << "\"";
        exit(1);
      } else {
        // DOC ou UNKNOWN
    ...
    string type_to_s() { return type_to_s(this->type); }
      ...
      if (this->type == DOC) {
        ...
        if (this->type == SWITCH) {
          ...
          if (idx != string::npos) {
            ...
            cerr << "id d'option incorrect : " << this->id
                 << " attendu xxx-val\n";
            exit(1);
          }

          buf << "  "
              << "--[no-]" << base_key << " : flag associé à  ";
          buf << this->id << " (def: " << this->value_to_s() << ")";
          /// buf << endl;
        } else {
 ...
 public:
  ...
  Option *add_long_option(string id, long &var) {
    Option *opt = add_option(id, LONG);
    opt->long_ref_variable = &var;
    return opt;
  }
  ...
  Option *add_abbrev_option(string id, string ref_option_id,
                            string value) {  // XXX A VIRER QUAND SERA FINI
    Option *opt = add_option(id, ABBREV);
    opt->active_value = value;
    if (!this->key_exists(ref_option_id)) {
      cerr << "Erreur : aucune option d'id " << ref_option_id
           << " n'est définie !";
      exit(1);
    }
  ...
  Option *add_switch_option(string id, bool &var) {
    ...
    // val_opt->abbrevs->at(0)->internal = true; // serait équivalent mais non
    // testé
  ...
  bool key_exists(string id) {
    map<string, OptionParser::Option *>::const_iterator it =
        this->map_options->find(id);
  ...
  // affiche pour chaque clé sa clé de référence (i.e. l'id de l'option
  // associée)
  string map_options_to_s() {
    stringstream buf;
    buf << "Contenu de map_options\n";
    map<string, OptionParser::Option *>::iterator it =
        this->map_options->begin();
  ...
  void on_error(string handler) {
    ...
    if (std::find(handlers.begin(), handlers.end(), handler) !=
        handlers.end()) {
  ...
  void send_error(string txt) {
    if (this->error_handler == "ignore") {
      ...
      cerr << "TODO: Valeur \"raise\" de error_handler non implantée :\n";
      cerr << "=> exit " << endl;
      cerr << this->abstract;
      cerr << endl;
      exit(1);
    ...
    for (unsigned i = 0; i < this->options->size(); i++) {
      ...
      for (unsigned i = 0; i < opt->abbrevs->size(); i++) {
        ...
        if (this->map_options->find(abbrev->id) != this->map_options->end()) {
          cerr << endl;
          cerr << "ERREUR : pour option#id=" << opt->id << endl;
          cerr << "  tentative de redéclaration de la clé " << abbrev->id
               << endl;
  ...
  virtual void parse() {
    ...
    while (i < argv.size()) {
      ...
      if (state == "NORMAL_STATE") {
        ...
        cerr << "ERREUR dans classe OptionParser : état inconnu state=" << state
             << endl;
        exit(1);
      }
    }
  ...
  virtual string inspect() {
    ...
    for (vector<Option *>::iterator it = options->begin(); it != options->end();
         it++) {
      buf << (*it)->inspect();
    }
    if (this->params.size() != 0) {
      buf << "Liste des " << this->params.size()
          << " arguments sans clé associés\n";
      ...
      buf << "Liste des fichiers à traiter vide (pas d'arguments "
             "supplémentaires\n";
    }

    return buf.str();
  }
  virtual ostream &print_dump(ostream &os = cout) {
    os << this->inspect();
    return os;
  }

  // Affiche la syntaxe de l'appli et la description des options
  virtual ostream &print_syntaxe(ostream &os = cout) {
  ...
  virtual ostream &print_values(ostream &os = cout) {
    ...
    if (this->params.size() == 0) {
      ...
      for (unsigned i = 0; i < this->params.size(); i++) {
        cout << "  Param " << i << " : " << this->params[i] << endl;
      }
    }
  ...
  virtual ostream &print_help(ostream &os = cout) { return print_syntaxe(); }
  // pre_parse_hook: permet un prétraitemenet avant d'extraire les options
  // (extraction du premier paramètre à la git ou cvs, ...)
  // À REDÉFINIR PAR LA CLASSE FILLE
  virtual void pre_parse_hook() {}
  // post_parse_hook: assure les vérification supplémentaire sur les options
  // extraites (validité des valeurs, cohérence pour les option
  // interdépendantes, ...) À REDÉFINIR PAR LA CLASSE FILLE
  virtual void post_parse_hook() {}
  ...
  static OptionParser *test(int argc, char *argv[]) {
    ...
    // TODO : implanter l'option --color et les switch d'abrévations
    string color = "blue";
    ...
    opts->on_error(
        "warn");  // possibles : exit (par défaut), raise, warn, ignore
