...
StupidSolver::StupidSolver(const Instance* inst) : Solver::Solver(inst) {

  logn2("StupidSolver START Résolution par algorithme glouton stupide.\n"
        " inst: " + inst->name);

  this->solution = new Solution(inst);
  logn2("StupidSolver END construit.");
}
StupidSolver::~StupidSolver() { delete this->solution; }
...
bool StupidSolver::solve() {
  if (log3()) {
    cout << "\n---StupidSolver::solve START instance: " << inst->name
         << " de taille " << inst->stations.size() << "\n";
  }
  ...
  for (unsigned j = 0; j < inst->stations.size(); j++) {
    ...
    Station* station = inst->stations[j];
